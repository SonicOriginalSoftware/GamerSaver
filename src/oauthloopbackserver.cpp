...
const QByteArray OAuthLoopbackServer::responseHTML{
    ...
    "<body>You may now close this window <script>"
      ...
      "fetch('/', {method:'POST', headers:{'Content-Type':'application/json'},body:JSON.stringify(r)});"
    "</script></body></html>"};

int OAuthLoopbackServer::GetListenPort() { return listenPort; }

QByteArray OAuthLoopbackServer::PromptForConsent(const QString& listenAddress, QEventLoop& loop)
{
  ...
  if (loopResult != 0 || connection == nullptr) return "";
